...
print("-----------------------------------------------------------------------------------")
print("-----------------------Comparativo Direto Entre os Modelos-------------------------")
print("-----------------------------------------------------------------------------------")

#Comparativo dos modelos com os valores de acurácia, recall, f1-score e precisão diretamente sem usar o "classification_report"
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score
#Definição da função para a avaliação
...
resultados = []
# Logistic Regression
resultados.append(avaliar_modelo("Logistic", y_test, y_pred))
# Random Forest
resultados.append(avaliar_modelo("Random Forest", y_test, y_pred_rf))
# XGBoost
resultados.append(avaliar_modelo("XGBoost", y_test, y_pred_xgb))
df_resultados = pd.DataFrame(resultados)
#Ordena pelo maior valor de recall de cada modelo
print(df_resultados.sort_values(by="Recall", ascending=False))
#------------------------------------------------------------------------------------------
#Gráfico Comparativo entre os Modelos
df_resultados.set_index("Modelo")[["Accuracy","Precision","Recall","F1-score"]].plot(kind="bar")
...
#--------------------------------------------------------------------------------------------
#Determinação do melhor modelo, com base no valor de Recall

melhor_modelo = df_resultados.sort_values(by="Recall", ascending=False).iloc[0]

print("\nMelhor modelo baseado em Recall:")
print(melhor_modelo)

#--------------------------------------------------------------------------------------------
#Comparativo dos valores de AUC de cada modelo 
auc_scores = {
    "Logistic": roc_auc_score(y_test, y_probs),
...
melhor_modelo = max(auc_scores, key=auc_scores.get)
print("\nMelhor modelo (AUC):", melhor_modelo)
print("AUC:", auc_scores[melhor_modelo])

#---------------------------------------------------------------------------------------
#Comparativo do gráficos ROC de cada modelo
...
for nome in auc_scores:  # reaproveitando diretamente as chaves
    fpr, tpr, _ = roc_curve(y_test, probs[nome])
    plt.plot(fpr, tpr, label=f"{nome} (AUC={auc_scores[nome]:.3f})")
